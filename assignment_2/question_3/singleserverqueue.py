...
def queue_hist(filename):
    ...
    service1_max = max(service1_list)
    service2_max = max(service2_list)

    hist1_range = np.arange(0, service1_max, 10)
    hist2_range = np.arange(0, service2_max, 10)
    inter_hist = np.histogram(interarrival_list, bins=bin_count)
    service1_hist = np.histogram(service1_list, bins=hist1_range)
    service2_hist = np.histogram(service2_list, bins=hist2_range)
    ...
    plt.hist(service1_hist, bins=hist1_range)
    plt.title("Service Time 1 Histogram")
    plt.savefig("service1.jpg")

    plt.hist(service2_hist, bins=hist2_range)
